...
public:
  std::map<std::string, llvm::AllocaInst *>
      namedValues; // Maybe should be Value*?
  std::map<std::string, Types *> namedTypes;
  llvm::BasicBlock *bblock;

  BlockContext(llvm::BasicBlock *bb) { this->bblock = bb; }
};
...
public:
  ...
  bool hasBlock() { return block.size() != 0; }
  ...
  Types *getType(std::string name) {
    for (std::vector<BlockContext *>::reverse_iterator i = block.rbegin();
         i != block.rend(); i++) {
...
public:
  Types *type;
  TypeNode(Types *type) { this->type = type; }
...
public:
  ...
  virtual llvm::Value *codegen(CompileContext *cc);
  virtual Types *resolveType(CompileContext *cc) { return new IntTypes(); }
};
...
public:
  ...
  RetNode(ExprNode *st) { this->st = st; }
...
public:
  ...
  VariableLoadNode(std::string name) { this->name = name; }
  virtual llvm::Value *codegen(CompileContext *cc);
...
enum class OP { PLUS, SUB, MULT, DIVIDE, EQUAL };
  ...
  FunctionSignature(std::string name, TypeNode *type,
    ...
    if (params) {
      for (auto arg : *params) {
        ...
        else if (arg->is_vararg) {
          llvm::errs()
              << "You can not have varargs in middle of signature, function "
              << name << "\n";
          exit(1);
        }
      }
