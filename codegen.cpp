...
llvm::Function *FunctionNode::codegen(CompileContext *cc) {
  ...
  if (declar) {
    ...
    if (!f) {
      llvm::errs() << "Attempt to define function without declaration, have "
                      "you forgotten a : ?\n At function "
                   << fs->name << "\n";
      exit(1);
      return nullptr;
  ...
  if (fs->params)
    for (auto &arg : *fs->params) {
      ...
      llvm::AllocaInst *alloc =
          TmpB.CreateAlloca(arg->type->codegen(cc), 0, arg->name);
...
llvm::Value *IntNode::codegen(CompileContext *cc) {
  // TODO is it 64?
  return llvm::ConstantInt::get(llvm::Type::getInt64Ty(cc->context), value,
                                true);
}
    ...
    if (expr) {
      ...
      if (this->type) {
        ...
        if (!(this->type->type->compatible(tmp))) {
          llvm::errs() << "Assigned expr not compatible with assigned type. "
                          "variable name: "
                       << name << "\n";
          exit(1);
          return nullptr;
      ...
      if (!this->type) {
        llvm::errs() << "We need either a type or an expr. variable name: "
                     << name << "\n";
        exit(1);
        return nullptr;
      }
...
llvm::Value *OpExprNode::codegen(CompileContext *cc) {
  ...
  return llvm::BinaryOperator::Create(instr, lval, rval, "",
                                      cc->getBlock()->bblock);
}
...
llvm::Value *WhileNode::codegen(CompileContext *cc) {
  ...
  llvm::BasicBlock *whileCondBB =
                       llvm::BasicBlock::Create(cc->context, "whilecond", f),
                   *whileBB = llvm::BasicBlock::Create(cc->context, "while"),
                   *whileEndBB =
                       llvm::BasicBlock::Create(cc->context, "whilecont");
...
llvm::Value *IfNode::codegen(CompileContext *cc) {
  ...
  llvm::BasicBlock *ifBB = llvm::BasicBlock::Create(cc->context, "if", f),
                   *elseBB,
